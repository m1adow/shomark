...
{
    private const long MaxVideoUploadBytes = 2L * 1024 * 1024 * 1024;
    private const long MultipartUploadHeadroomBytes = 64L * 1024 * 1024;
    private const long MaxMultipartUploadRequestBytes = MaxVideoUploadBytes + MultipartUploadHeadroomBytes;
    ...
    [HttpPost("upload")]
    [RequestSizeLimit(MaxMultipartUploadRequestBytes)]
    [RequestFormLimits(MultipartBodyLengthLimit = MaxMultipartUploadRequestBytes)]
    public async Task<IActionResult> Upload(IFormFile file, CancellationToken ct)
    {
        ...
        if (file.Length > _videoOptions.MaxFileSizeBytes)
            return BadRequest(new { Error = $"Video file size cannot exceed {_videoOptions.MaxFileSizeBytes} bytes", ErrorCode = Constants.Errors.Codes.Validation });
