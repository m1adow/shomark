...
public class AiFragmentService : IAiFragmentService
{
    ...
    public async Task<Result<AiFragmentDto>> UpdateAsync(Guid id, UpdateAiFragmentRequest request, CancellationToken ct = default)
    {
        ...
        var wasApproved = fragment.IsApproved;
        ...
        if (request.IsApproved == true && !wasApproved)
        {
            await _eventPublisher.PublishAsync(
                KafkaTopics.FragmentApproved,
                new FragmentApprovedEvent(
                    fragment.Id,
                    fragment.VideoId,
                    _currentUser.UserId,
                    ...
                    fragment.EndTime,
                    fragment.StorageKey,
                    fragment.CalculateViralScore(),
                    fragment.Hashtags,
                    fragment.ThumbnailKey,
                    DateTime.UtcNow),
                ct);
        }
