...
public class PostPublishingService : IPostPublishingService
{
    ...
    public async Task<Result<PostDto>> PublishPostAsync(Guid postId, CancellationToken ct = default)
    {
            ...
            tokens.TokenExpiresAt.Value <= DateTime.UtcNow.AddMinutes(5))
        {
            ...
            {
                await MarkPostFailedAsync(post, tokens, Constants.Errors.Messages.TokenExpiredNoRefresh, ct);
            ...
            catch (Exception ex)
            {
                ...
                await MarkPostFailedAsync(post, tokens, $"Token refresh failed: {ex.Message}", ct);
        ...
        if (post.FragmentStorageKey is not null)
        {
            var parts = post.FragmentStorageKey.Split('/', 2);
            if (parts.Length == 2)
            {
        ...
        {
            await MarkPostFailedAsync(post, tokens, $"No publisher available for {tokens.PlatformType}", ct);
        ...
        {
            await MarkPostFailedAsync(post, tokens, result.ErrorMessage ?? Constants.Errors.Messages.UnknownPublishingError, ct);
    ...
    private async Task MarkPostFailedAsync(Domain.Entities.Post post, DecryptedPlatformTokens tokens, string error, CancellationToken ct)
    {
        post.Status = PostStatus.Failed;
        await _postRepository.UpdateAsync(post, ct);
        await _eventPublisher.PublishAsync(
            KafkaTopics.PostFailed,
            new PostFailedEvent(
                post.Id,
                tokens.UserId,
                tokens.PlatformId,
