...
/// whose ScheduledAt time has passed, then publishes them through Social API.
/// </summary>
public class PostSchedulerBackgroundService : BackgroundService
{
    ...
    private async Task ProcessScheduledPostsAsync(CancellationToken ct)
    {
        ...
        foreach (var post in duePosts)
        {
            ...
            try
            {
                var result = await publishingService.PublishPostAsync(post.Id, ct);
                if (result.IsSuccess)
                {
                ...
                {
                    _logger.LogWarning("Scheduled post {PostId} publishing failed: {Error}", post.Id, result.Error);
                }
            }
            catch (Exception ex)
            {
